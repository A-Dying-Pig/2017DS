...
public:
    ...
    void _itoa (int i);
    void clear();
    int compare_date (m_charstring<T> src);        //比较发帖日期前后,0一样大，1是自身大,-1是src大
    T* _str();
    T pop_back();
    T back();
};
...
template <typename T>
int m_charstring<T>::compare_date(m_charstring<T> src)
{
    int d1[3]= {0,0,0},d2[3]={0,0,0},count = 0;
    char temp[10];
